            ...
            <div class="space-y-3">
                @forelse ($report['packages'] as $package)
                    <div class="rounded-lg border border-gray-200 p-3 dark:border-white/10">
                        <div class="flex flex-wrap items-center justify-between gap-2"><span class="font-medium">{{ $package['name'] }}</span><span class="text-sm">@switch($package['status'])@case('compatible')<span class="text-success-600">{{ trans('minecrafttoolkit::strings.version_change.compatible') }}</span>@break @case('update_required')<span class="text-warning-600">{{ trans('minecrafttoolkit::strings.version_change.update_required') }}</span>@break @case('system_update')<span class="text-primary-600">{{ trans('minecrafttoolkit::strings.version_change.system_update') }}</span>@break @case('incompatible')<span class="text-danger-600">{{ trans('minecrafttoolkit::strings.version_change.incompatible') }}</span>@break @default<span class="text-gray-500">{{ trans('minecrafttoolkit::strings.version_change.unknown') }}</span>@endswitch</span></div>
