...
class BlueItAnnouncementService
{
    ...
    {
        if (! Schema::hasTable('minecraft_toolkit_announcement_reads')) {
            return;
        }

        DB::table('minecraft_toolkit_announcement_reads')->updateOrInsert(
    ...
    public function wasDelivered(User $user, string $announcementId): bool
    {
        ...
        return DB::table('minecraft_toolkit_announcement_reads')
    ...
    public function removeInactiveDatabaseNotifications(User $user, array $announcements): void
    {
        ...
        $user->notifications()
            ...
            ->each(function ($notification) use ($activeIds, $knownIds): void {
                ...
                $belongsToPlugin = $pluginId === 'minecrafttoolkit'
    ...
    private function notification(array $announcement): Notification
    {
        ...
        $buttonText = (string) ($announcement['button_text'] ?? trans('minecrafttoolkit::strings.announcements.open'));
        ...
        $notification = Notification::make((string) $announcement['notification_id'])
            ...
            ->viewData([
                'blueit_plugin_id' => 'minecrafttoolkit',
        ...
        if ($buttonUrl) {
            $notification->actions([
                Action::make('open_blueit_minecrafttoolkit_announcement_'.preg_replace('/[^a-zA-Z0-9_]/', '_', (string) $announcement['id']))
                    ->button()
                    ->label($buttonText)
                    ->markAsRead()
                    ->url($buttonUrl),
    ...
    {
        if (! $user || ! (bool) config('minecrafttoolkit.blueit_announcements_enabled', true)) {
            return false;
        }
            ...
            return $user->servers()->exists()
                || $user->subusers()
                    ...
                    ->contains(fn ($subuser): bool => in_array(
                        'minecrafttoolkit.announcements',
                        (array) $subuser->permissions,
                        true
                    ));
        }

        return $server->owner_id === $user->id || $user->can('minecrafttoolkit.announcements', $server);
    }
    ...
    {
        $url = (string) config('minecrafttoolkit.blueit_announcements_url', '');
        if ($url === '') {
            return ['available' => false, 'announcements' => []];
        }

        return Cache::remember('minecrafttoolkit.blueit_announcements', 10, function () use ($url): array {
            try {
                $response = Http::timeout((int) config('minecrafttoolkit.http_timeout', 20))
                    ->acceptJson()
                    ->get($url, [
                        'plugin_id' => 'minecrafttoolkit',
                        'version' => self::PLUGIN_VERSION,
                        'panel_version' => '1.0.0-beta34',
                    ]);
                ...
                Log::debug('Minecraft Toolkit BlueIT announcements unavailable.', [
    ...
    {
        $secret = (string) config('minecrafttoolkit.blueit_announcements_secret', '');
        if ($secret === '') {
            return true;
        }
    ...
    private function localize(array $announcement): array
    {
        ...
        $announcement['button_text'] = $this->localizedValue($announcement['button_label'] ?? [], $locale)
            ?: trans('minecrafttoolkit::strings.announcements.open');
        ...
        $announcement['notification_id'] = 'blueit-minecrafttoolkit-'.hash('sha256', $announcement['read_id']);

        $tenant = Filament::getTenant();
        if (($announcement['type'] ?? 'normal') === 'update' && ! $announcement['button_url'] && $tenant instanceof Server) {
            $announcement['button_url'] = MinecraftUpdaterPage::getUrl(panel: 'server', tenant: $tenant);
        }
